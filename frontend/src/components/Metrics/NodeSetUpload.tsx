...
async function uploadNodeSetXML(files: File[]) : Promise<MetricsNodeSetProviderResponse> {
...
export function useUploadNodeSet() {
  ...
  return useMutation<MetricsNodeSetProviderResponse, Error, File[]>({
    ...
    return (
        <div>
            ...
            {missingDeps.length === 0 && namespaces.length > 0 && (
                <ChartSection namespaces={namespaces} />
            )}
        </div>
    );
}
