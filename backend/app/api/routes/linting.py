...
from app.services.linting_handling import *
from app.api.deps import SessionDep
from app.models import LintingAnalyzeResponse, LintingNodesetProvideResponse, CsvFile
from fastapi.responses import StreamingResponse


router = APIRouter(prefix="/linting", tags=["linting"])
...
async def provide(session: SessionDep,
                  ...
                  additional_nodeset_files: list[UploadFile] = Depends(parse_additional_nodesets)) -> LintingNodesetProvideResponse:
    
    cleanup()
    ensure_linting_tool_up_to_date()
    ...
    return LintingNodesetProvideResponse(
...
@router.get("/analyze")
def analyze(namespace_uri: str) -> LintingAnalyzeResponse:
    ...
    run_linter(namespace_uri)

    csv_files = [CsvFile(**f) for f in collect_csv_files()]

    return LintingAnalyzeResponse(
...
def download() -> StreamingResponse:
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": 'attachment; filename="linting.zip"'
        },
    )
