...
# --- Pfade ---
METRICS_TEMP_DIR = Path("metrics_temp")
WORK_DIR = METRICS_TEMP_DIR / "upload"
XMLS_DIR = WORK_DIR / "xmls"
CSV_DIR = WORK_DIR / "csv"
DEPENDENCY_CACHE = METRICS_TEMP_DIR / "dependencies"
TOOLS_DIR = Path("tools")
METRICS_BINARY = TOOLS_DIR / "Metrics"
...
def ensure_metrics_tool_up_to_date():
    """Stellt sicher, dass das mitgelieferte Metrics-Tool vorhanden ist."""
    if not METRICS_BINARY.exists():
        raise RuntimeError(f"Metrics-Tool nicht verfügbar: {METRICS_BINARY}")

    METRICS_BINARY.chmod(0o755)
...
def run_metrics(namespace_uri: str):
    """Ruft das C#-Metrics-Tool auf."""
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Metrics-Tool Fehler: {result.stderr}")
